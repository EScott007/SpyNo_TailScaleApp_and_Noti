...
TAILSCALE_HOST = os.getenv("APP_HOST", "100.117.253.67")
APP_PORT = int(os.getenv("APP_PORT", "5001"))
MOCK_MODE = os.getenv("MOCK_MODE", "0") == "1"
BRIDGE_STALE_SECONDS = float(os.getenv("BRIDGE_STALE_SECONDS", "3.0"))
AUX_API_PORT = int(os.getenv("AUX_API_PORT", "5000"))
...
def mjpeg_generator():
    api_url = f"http://localhost:{AUX_API_PORT}/video/left"
    while True:
        try:
            with requests.get(api_url, stream=True, timeout=(3, 10)) as r:
                # Use chunk_size=1024 instead of 4096 to avoid buffering
                # an entire frame before yielding — important against an
                # eventlet-backed source that sleeps between frames.
...
<head>
    ...
    <style>
        ...
        #pipeline-btn { width: 100%; padding: 10px; font-family: 'Courier New', monospace; font-size: 14px; font-weight: bold; text-transform: uppercase; cursor: pointer; border: 2px solid #00f2ff; background: rgba(0, 242, 255, 0.1); color: #00f2ff; letter-spacing: 2px; transition: 0.2s; }
        #pipeline-btn:hover:not(:disabled) { background: rgba(0, 242, 255, 0.25); }
        #pipeline-btn:disabled { opacity: 0.4; cursor: not-allowed; }
        #pipeline-btn.running { border-color: #ff3e3e; color: #ff3e3e; background: rgba(255, 62, 62, 0.1); }
        #pipeline-btn.running:hover:not(:disabled) { background: rgba(255, 62, 62, 0.25); }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="main-layout">
        ...
        <div class="data-sector">
            ...
            <button id="pipeline-btn" onclick="togglePipeline()">⬤ START CAMERA</button>
    ...
    <script>
        ...
        async function togglePipeline() {
            ...
            btn.disabled = true;
            const endpoint = _pipelineRunning ? '/api/pipeline/stop' : '/api/pipeline/start';
            try {
                const r = await fetch(endpoint, { method: 'POST' });
                if (r.status === 429) {
                    btn.innerText = '⏳ COOLING DOWN...';
                    btn.classList.remove('running');
                    setTimeout(() => { btn.disabled = false; refreshPipelineState(); }, 3000);
                    return;
                }
                if (r.status === 502 || r.status === 504) {
                    btn.innerText = '⚠ PIPELINE OFFLINE';
                    btn.classList.remove('running');
                    setTimeout(() => { btn.disabled = false; refreshPipelineState(); }, 4000);
                    return;
                }
            } catch(e) {
                console.error('Pipeline toggle error:', e);
                btn.innerText = '⚠ NETWORK ERROR';
                btn.classList.remove('running');
                setTimeout(() => { btn.disabled = false; refreshPipelineState(); }, 4000);
                return;
            }
            setTimeout(() => { btn.disabled = false; refreshPipelineState(); }, 1500);
        }
        ...
        async function refreshPipelineState() {
            try {
                ...
                _pipelineRunning = d.running;
                const btn = document.getElementById('pipeline-btn');
                if (d.cooldown) {
                    btn.disabled = true;
                    btn.innerText = '⏳ COOLING DOWN...';
                    btn.classList.remove('running');
                    setTimeout(refreshPipelineState, 1000);
                } else if (d.running) {
                    btn.disabled = false;
                ...
                // Pipeline unreachable — re-enable button so user isn't stuck
                const btn = document.getElementById('pipeline-btn');
                btn.disabled = false;
                btn.classList.remove('running');
            }
        ...
        // Poll pipeline state every 2 seconds to stay in sync
        setInterval(refreshPipelineState, 2000);
...
def pipeline_start():
    try:
        r = requests.post(f'http://localhost:{AUX_API_PORT}/pipeline/start', timeout=3)
        return jsonify(r.json()), r.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"status": "error", "message": "aux_f pipeline unreachable"}), 502
    except requests.exceptions.Timeout:
        return jsonify({"status": "error", "message": "aux_f pipeline timeout"}), 504
    except Exception as e:
...
def pipeline_stop():
    try:
        r = requests.post(f'http://localhost:{AUX_API_PORT}/pipeline/stop', timeout=3)
        return jsonify(r.json()), r.status_code
    except requests.exceptions.ConnectionError:
        return jsonify({"status": "error", "message": "aux_f pipeline unreachable"}), 502
    except requests.exceptions.Timeout:
...
def pipeline_state():
    try:
        r = requests.get(f'http://localhost:{AUX_API_PORT}/pipeline/state', timeout=3)
        return jsonify(r.json()), r.status_code
    except Exception:
        return jsonify({"running": False, "ready": False, "cooldown": False}), 200
